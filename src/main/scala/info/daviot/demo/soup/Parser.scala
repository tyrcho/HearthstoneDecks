...
object Parser extends App with Logging {
  ...
  val hstdScraper = new HsTopDecksScraper(Seq("http://www.hearthstonetopdecks.com/deck-category/constructed-seasons/season-14/"), cacheFolder, "14")
  val hstdDecks = Await.result(hstdScraper.collectedData, 1000.seconds).values.filter(_.hero == "Druid")
